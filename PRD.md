...
3. 详细功能需求 (Functional Requirements)
  ...
  取价算法:
  获取过滤后的前 N 个最优广告单（N默认=5，可配置）。
  数据存储: 始终存储采集到的原始 Top N 数据（即 Rank 1 到 Rank N 的每一条价格记录），不进行平均值预计算，以便保留完整市场深度数据供后续分析。
  B. 外汇汇率采集
  ...
  示例: 汇率 7.20，阈值 0.5%。如果 USDT 价格 < 7.164，则触发报警。
  报警方式:
  ...
  C2C 价格表 (c2c_prices)
   ...
   target_amount (INT): 关键字段。表示该价格对应的交易金额档位 (e.g., 30, 200, 500)。0 代表无金额限制（最低价）。
...
monitor:
  ...
  forex_interval_hours: 1
  alert_threshold_percent: 0.5 # 0.5% difference
